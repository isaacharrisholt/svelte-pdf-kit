...
export class PDF<TProps extends Record<string, any>> {
	...
	private async handleText(
		...
		pdf.text(he.decode(node.textContent), parsedOptions)
		return { data: null, error: null }
	}
	...
	private async handleImage(
		...
		// TODO: type
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		const { data: parsedOptions, error: optionParseError } =
			this.parseJsonFromHtml<any>(options)
		if (optionParseError) {
	...
	private async handleFont(node: DOMParser.Node): Promise<PDFResult<null>> {
		...
		const { data: fontDefinitions, error: fontDefinitionParseError } =
			this.parseJsonFromHtml<FontDefinition[]>(fonts)
