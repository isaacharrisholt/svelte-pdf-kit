...
import { type FontWeight, type FontStyle, type FontDefinition, type PDFResult, DEFAULT_FONT_STYLE, DEFAULT_FONT_WEIGHT, FONT_WEIGHTS, type FontWeightNumber } from "./types";
...
export class Fonts {
  ...
  public registerFont(
    ...
    if (Array.isArray(fonts)) {
      ...
      if (existingFamily) {
        fonts.forEach((font) => {
          existingFamily.registerVariant(font.style ?? DEFAULT_FONT_STYLE, font.weight ?? DEFAULT_FONT_WEIGHT, font.src)
        })
        return
      }
    ...
    const existingFamily = this.families.find((f) => f.name === family)
    if (existingFamily) {
      existingFamily.registerVariant(fonts.style ?? DEFAULT_FONT_STYLE, fonts.weight ?? DEFAULT_FONT_WEIGHT, fonts.src)
      return
    }
  ...
  public async getFontFile(
    ...
    return await fontFamily.getFontFile(style ?? DEFAULT_FONT_STYLE, weight ?? DEFAULT_FONT_WEIGHT, fetch)
  }
  ...
  public getFontName(
    ...
    return `${family}-${style ?? DEFAULT_FONT_STYLE}-${Fonts.weightToNumber(weight ?? DEFAULT_FONT_WEIGHT)}`
  }
}
